...
Since migrating a few of my projects to [Yarn Modern][yarn],
I have been struggling with speeding up the dependency installation process on [GitHub Actions][github-actions].

From the beginning, I have been using the great [`bahmutov/npm-install`][npm-install] action
that handles the caching of `node_modules` for Yarn v1.22.22 very well.
...
And in one of their [most recent release][setup-node-with-cache]
I discovered that they have added a new feature for [caching the package manager's cache][setup-node-caching-deps].
...
But this approach caches only the package managers global cache like `~/.npm` or `~/.yarn/cache`.
Additionally using [`actions/cache`][caching-dependencies] another layer of the caching can be added.
So state of the `node_modules` can be cached and restored to speed up the pipeline even more.
...
[caching-dependencies]: https://github.com/actions/cache/blob/main/caching-strategies.md
...
[setup-node-caching-deps]: https://github.com/actions/setup-node/blob/main/docs/advanced-usage.md#caching-packages-dependencies
